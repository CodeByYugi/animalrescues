...
maybe add ratio breakddown by year (dog/cat/other)

```{python}
data.groupby([data.fin_year, 'animal'], dropna=False).size().unstack(1, fill_value=0).plot()
```

incident distribution by animal type (overall)
```{python}
data.groupby('animal', dropna=False).size().sort_values(ascending=False).to_frame().assign(ratio=lambda x: x/x.sum(), cumratio=lambda x:x.ratio.cumsum())
```
